...
fn main() {
    ...
    match cli.command {
        Commands::Build => {
            ...
            posts_collected.sort_by(|a, b| {
                b.front_matter.date.cmp(&a.front_matter.date)
            });

            // Generate posts with navigation context
            for (index, post) in posts_collected.iter().enumerate() {
                let donation_section =
                    generate_donation_section(&config);

                let navigation_section =
                    generate_post_navigation(&posts_collected, index);
                ...
                let final_html = wrap_in_template(
                    ...
                    post.front_matter
                        ...
                        .as_deref()
                        .unwrap_or(
                            "A blog post generated by Xeniria — a fast, minimal Rust-based static site generator. https://github.com/0xh4ty/xeniria"
                        ),
                    &post_body,
                    "../",
                    post_url,
...
fn generate_post_navigation(
    posts: &[Post],
    current_index: usize,
) -> String {
    let newer_post = if current_index > 0 {
        Some(&posts[current_index - 1])
    } else {
        None
    };

    let older_post = posts.get(current_index + 1);

    let newer_html = if let Some(post) = newer_post {
        let link = post.file_name.replace("docs/posts/", "");

        format!(
            "<a href='{link}' class='nav-button nav-prev'>
                ← Previous
            </a>",
            link = link
        )
    ...
    let older_html = if let Some(post) = older_post {
        let link = post.file_name.replace("docs/posts/", "");

        format!(
            "<a href='{link}' class='nav-button nav-next'>
                Next →
            </a>",
            link = link
        )
...
<footer class='container' style='text-align: center;'>
    ...
    <p class='footer-credit'>
        © {copyright_year} <a href='{twitter}'>{author}</a>. Powered by <a href='https://github.com/0xh4ty/xeniria'>Xeniria</a>.
    </p>
</footer>
<script>
  function toggleMenu() {{
